...
class SOMUSIC_CLASS_CompositionUtil {
	...
	private function getNoteDistance($note, $modal) {
		...
		$note = $minMax[0];
		while($note["octave"]!=$minMax[1]["octave"] || $note["step"]!=$minMax[1]["step"]) {
			if($note["step"]=="B" || $note["step"]=="E")
				$distance++;
			else $distance+=2;
			$stepValue = $this->stepValue[$note["step"]];
			if($stepValue==6) {
				$note["step"] = "C";
				$note["octave"]++;
			}
			else $note["step"] = array_keys($this->stepValue, $stepValue+1)[0];
		}
