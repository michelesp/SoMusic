...
class SOMUSIC_CTRL_Editor extends OW_ActionController {
	 ...
	 * if(empty($_REQUEST["instrumentsUsed"]) || empty($_REQUEST["timeSignature"]) || empty($_REQUEST["keySignature"]))
	 * exit (json_encode (array('error' => "error initializzation editor")));
	 * $userId = OW::getUser()->getId();
	 * $cache = new Memcached();
	 * $cache->addServer("localhost", 11211);
	 * $composition = new SOMUSIC_CLASS_Composition(-1, "", $userId, -1, $userId, -1, array(), $_REQUEST["instrumentsUsed"]);
	 ...
	 * array_push($is->measures, $this->newMeasure($instrument["scoresClef"][$i], explode("/", $_REQUEST["timeSignature"]), $_REQUEST["keySignature"]));
	 ...
	 * }
	 */
	public function initEditor($instrumentsUsed, $timeSignature, $keySignature) { // TODO: bloccare chiamata rest
		$userId = OW::getUser ()->getId ();
		$cache = new Memcached ();
		$cache->addServer ( "localhost", 11211 );
		$composition = new SOMUSIC_CLASS_Composition ( - 1, "", $userId, - 1, $userId, - 1, array (), $instrumentsUsed );
	...
	public function setComposition($composition) { // TODO: bloccare chiamata rest
		$userId = OW::getUser ()->getId ();
		$cache = new Memcached ();
		$cache->addServer ( "localhost", 11211 );
		$cache->set ( $userId, $composition );
	...
	public function addNote() {
		...
		if ($note->isRest) {
			...
			if ($_REQUEST ["isPause"] == "true")
				...
				array_unshift ( $toAdd, new SOMUSIC_CLASS_Note ( - 1, array (
						$newNote [0] 
				), array (
						$newNote [1] 
				), null, $duration, false, array (), array () ) );
	...
	public function addTie() {
		...
		if ($tieIndex < 0) {
			...
			$this->removeTie ( $instrumentScore, $tieIndex );
		$cache->set ( $userId, $composition );
		exit ( json_encode ( $composition ) );
	}
	...
	private function getCompositionObject($compositionArray) {
		...
		foreach ($compositionArray["instrumentsScore"] as $instrumentScoreArray) {
			$instrumentScore = new SOMUSIC_CLASS_InstrumentScore($instrumentScoreArray["id"], $instrumentScoreArray["default_clef"], $instrumentScoreArray["name"], array(), array(), $instrumentScoreArray["instrument"]);
