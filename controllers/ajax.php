...
class SOMUSIC_CTRL_Ajax extends NEWSFEED_CTRL_Ajax {
	public function addComment() {
		...
		if (BOL_TextFormatService::getInstance ()->isCommentsRichMediaAllowed () && ! $isMobile) {
			if (! empty ( $clean ['attachmentInfo'] )) {
				...
				OW::getEventManager ()->call ( 'base.attachment_save_image', array (
						'uid' => $tempArr ['uid'],
						'pluginKey' => $tempArr ['pluginKey'] 
				) );
	...
	public function statusUpdate() {
		...
		if (! empty ( $data )) {
			...
			$item = empty ( $data ["entityType"] ) || empty ( $data ["entityId"] ) ? null : array (
		...
		$out = NEWSFEED_BOL_Service::getInstance ()->addStatus ( OW::getUser ()->getId (), $clean ['feedType'], $clean ['feedId'], $clean ['visibility'], $status, array (
				"content" => $content,
